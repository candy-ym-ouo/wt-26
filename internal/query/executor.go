...
func NewExecutor(engine *storage.Engine) *Executor {
	return &Executor{engine: engine, labels: make(map[uint64]map[string]string)}
}
...
func (e *Executor) collect(request model.QueryReq) ([]model.Series, map[uint64][]model.Point) {
	...
	for index, item := range series {
		if cached, ok := e.labels[item.ID]; ok {
			series[index].Tags = cached
		} else {
			e.labels[item.ID] = item.Tags
		}
